...
const STEPS = ['Search', 'Select Train', 'Seats', 'Details', 'Review'];

const CITIES = ['Delhi','Mumbai','Bangalore','Chennai','Kolkata','Hyderabad','Ahmedabad','Pune','Jaipur','Lucknow','Kochi','Varanasi'];

/* ── Train result card ───────────────────────────────────── */
const TrainCard = ({ train, selected, onSelect }) => (
  <button
  ...
  >
    <div className="flex items-center justify-between flex-wrap gap-4">
      <div className="flex items-center gap-6">
        ...
        <div>
          <div className="font-black text-stone-900 text-sm" style={{ fontFamily: 'var(--font-display)' }}>{train.trainName}</div>
          <div className="text-xs text-stone-400">{train.trainNumber}</div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>{train.departureTime}</div>
            <div className="text-xs text-stone-400">{train.from}</div>
          </div>
          <div className="flex flex-col items-center px-3">
            <div className="text-[10px] text-stone-400 mb-1">{train.duration}</div>
            <div className="flex items-center gap-1">
              <div className="w-12 h-px bg-stone-300" />
              <div className="w-1.5 h-1.5 rounded-full bg-orange-500" />
              <div className="w-12 h-px bg-stone-300" />
            </div>
            <div className="text-[10px] text-stone-400 mt-1">{train.type}</div>
          </div>
          <div>
            <div className="text-xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>{train.arrivalTime}</div>
            <div className="text-xs text-stone-400">{train.to}</div>
          </div>
        </div>
      </div>
      <div className="text-right">
        ...
        <div className="text-xs text-stone-400">{train.class}</div>
        <div className="text-xs text-stone-400 mt-0.5">{train.availableSeats} seats left</div>
      </div>
    </div>
  </button>
);
...
const TrainBookingPage = () => {
  ...
  const [search, setSearch] = useState({
    ...
    class: location.state?.searchParams?.class || '3A',
  });
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await apiService.searchTrains(search);
      if (res.success) { setTrains(res.trains); setStep(1); }
    } catch { setError('Failed to search trains. Please try again.'); }
    ...
    <div className="min-h-[calc(100vh-68px)] bg-stone-50 pb-20 md:pb-8 page-enter">
      ...
      <div className="bg-white border-b border-stone-100">
        <div className="max-w-[1000px] mx-auto px-5 md:px-10 py-6">
          <h1 className="text-2xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>🚂 Train Booking</h1>
          <p className="text-stone-500 text-sm mt-1">Search and book train tickets across India</p>
        </div>
      </div>
      ...
      <div className="max-w-[1000px] mx-auto px-5 md:px-10 py-8">
        ...
        {step === 0 && (
          <Card padding>
            <h2 className="font-bold text-stone-900 mb-5" style={{ fontFamily: 'var(--font-display)' }}>Search Trains</h2>
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select label="Class" value={search.class} onChange={e => setSearch(p=>({...p,class:e.target.value}))}>
                  {['Sleeper (2S)','3rd AC (3A)','2nd AC (2A)','1st AC (1A)','Chair Car (CC)'].map(c=><option key={c} value={c.split(' ')[0]}>{c}</option>)}
                </Select>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>
                {trains.length} trains found · {search.from} → {search.to}
              </h2>
              <Button variant="ghost" size="sm" onClick={() => setStep(0)}>← Edit Search</Button>
            </div>
            {trains.map(t => (
              <TrainCard key={t.id} train={t} selected={selected?.id === t.id} onSelect={handleSelectTrain} />
            ))}
          </div>
        )}
          ...
          <div className="space-y-5">
            ...
            <Card padding>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-stone-500">Train</span><span className="font-semibold">{selected.trainName} · {selected.trainNumber}</span></div>
