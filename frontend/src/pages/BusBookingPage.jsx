...
const STEPS = ['Search', 'Select Bus', 'Seats', 'Details', 'Review'];

const CITIES = ['Delhi','Mumbai','Bangalore','Chennai','Kolkata','Hyderabad','Ahmedabad','Pune','Jaipur','Lucknow','Goa'];

/* ── Bus result card ───────────────────────────────────── */
const BusCard = ({ bus, selected, onSelect }) => (
  <button
  ...
  >
    <div className="flex items-center justify-between flex-wrap gap-4">
      <div className="flex items-center gap-6">
        ...
        <div>
          <div className="font-black text-stone-900 text-sm" style={{ fontFamily: 'var(--font-display)' }}>{bus.operator}</div>
          <div className="text-xs text-stone-400">{bus.busType}</div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>{bus.departureTime}</div>
            <div className="text-xs text-stone-400">{bus.from}</div>
          </div>
          <div className="flex flex-col items-center px-3">
            <div className="text-[10px] text-stone-400 mb-1">{bus.duration}</div>
            <div className="flex items-center gap-1">
              <div className="w-12 h-px bg-stone-300" />
              <div className="w-1.5 h-1.5 rounded-full bg-orange-500" />
              <div className="w-12 h-px bg-stone-300" />
            </div>
            <div className="text-[10px] text-stone-400 mt-1">Non-stop</div>
          </div>
          <div>
            <div className="text-xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>{bus.arrivalTime}</div>
            <div className="text-xs text-stone-400">{bus.to}</div>
          </div>
        </div>
      </div>
      <div className="text-right">
        ...
        <div className="text-xs text-stone-400">{bus.class}</div>
        <div className="text-xs text-stone-400 mt-0.5">{bus.availableSeats} seats left</div>
      </div>
    </div>
  </button>
);
        ...
        {Array.from({ length: rows }, (_, row) => (
          <div key={row} className="flex items-center justify-center gap-1 mb-1">
            {cols.map((col, ci) => {
              ...
              const occupied = ['1A','2C','3D','4B','5A'].includes(seat);
...
const BusBookingPage = () => {
  ...
  const [search, setSearch] = useState({
    ...
    class: location.state?.searchParams?.class || 'Sleeper',
  });
  const [buses, setBuses] = useState([]);
  const [selected, setSelected] = useState(null);
  const [seats, setSeats] = useState([]);
  const [details, setDetails] = useState({ firstName:'', lastName:'', email: user?.email||'', phone: user?.phone||'', dob:'', gender:'Male' });
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await apiService.searchBuses(search);
      if (res.success) { setBuses(res.buses); setStep(1); }
    ...
    <div className="min-h-[calc(100vh-68px)] bg-stone-50 pb-20 md:pb-8 page-enter">
      ...
      <div className="bg-white border-b border-stone-100">
        <div className="max-w-[1000px] mx-auto px-5 md:px-10 py-6">
          <h1 className="text-2xl font-black text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>🚌 Bus Booking</h1>
          <p className="text-stone-500 text-sm mt-1">Search and book bus tickets across India</p>
        </div>
      </div>
      ...
      <div className="max-w-[1000px] mx-auto px-5 md:px-10 py-8">
        ...
        {step === 0 && (
          <Card padding>
            <h2 className="font-bold text-stone-900 mb-5" style={{ fontFamily: 'var(--font-display)' }}>Search Buses</h2>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-stone-900" style={{ fontFamily: 'var(--font-display)' }}>
                {buses.length} buses found · {search.from} → {search.to}
              </h2>
              <Button variant="ghost" size="sm" onClick={() => setStep(0)}>← Edit Search</Button>
            </div>
            {buses.map(b => (
              <BusCard key={b.id} bus={b} selected={selected?.id === b.id} onSelect={handleSelectBus} />
            ))}
          </div>
        )}
        ...
        {step === 2 && selected && (
          <div className="space-y-6">
            ...
            <Card padding>
              ...
              <SeatMap selected={seats} onToggle={(seat) => {
        ...
        {step === 3 && (
          <Card padding>
            ...
            <form onSubmit={handleDetailsNext} className="space-y-4 max-w-lg">
              ...
              <Select label="Gender" value={details.gender} onChange={e=>setDetails(p=>({...p,gender:e.target.value}))}>
          ...
          <div className="space-y-5">
            ...
            <Card padding>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-stone-500">Bus</span><span className="font-semibold">{selected.operator} · {selected.busType}</span></div>
